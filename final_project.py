...
def process_university_search(command):
    ...
    if major and tuition and state:
        results = session.query(University,func.count(University.name)).join(Location).join(Major).filter(Major.major.like(major)).filter(University.tuition <= tuition).filter(University.tuition>0.0).filter(Location.state == state).group_by(Major.name).all()
    ...
    # if major and state are specified
        ...
        results = session.query(University,func.count(University.name)).join(Location).join(Major).filter(Major.major.like(major)).filter(Location.state == state).all()
        return results[0:limit]
